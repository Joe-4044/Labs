...
2. (*8 points*) Recreate a game map that includes three distinct obstacle walls using a ```TileMapLayer```. You will have to us the provided `sheet.png` with your ```TileMapLayer```. Use one tile for the background. Use another tile that matches aesthetically for the obstacles (like the flowers). The tank should not pass through the obstacle.
